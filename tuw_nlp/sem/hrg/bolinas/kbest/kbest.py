...
class KBestBolinasScript(BolinasScript):

    def run_loop(self):
        logprob = True

        score_disorder_collector = {}
        model_dir = os.path.join(self.data_dir, self.config["model_dir"])

        for sen_idx in get_range(model_dir, self.first, self.last):
            ...
            gold_labels = get_gold_labels(f"{self.data_dir}/{self.config['preproc_dir']}", sen_idx)
            top_order = json.load(open(
                f"{self.data_dir}/{self.config['preproc_dir']}/{sen_idx}/preproc/pos_edge_graph_top_order.json"
            ))
            pos_tags = get_pos_tags(f"{self.data_dir}/{self.config['preproc_dir']}/{sen_idx}/preproc/parsed.conll")
...
if __name__ == "__main__":
    ...
    script = KBestBolinasScript(
        args.data_dir,
        args.config,
        log_file_prefix=f"{os.path.dirname(os.path.realpath(__file__))}/log/kbest_"
    )
    script.run()
