...
from tuw_nlp.sem.hrg.bolinas.common.exceptions import DerivationException, GrammarError, ParserError
from tuw_nlp.sem.hrg.bolinas.common.hgraph.hgraph import Hgraph
from tuw_nlp.sem.hrg.bolinas.common.logarithm import LOGZERO, logadd
...
class Grammar(dict):
            ...
            if l:
                ...
                if ";" in content:
                    ...
                    try:
                        ...
                        raise GrammarError("Line %i, Rule %i: Could not initialize rule. %s" % (line_count, rule_count, e))
                    buf = StringIO()
                    rule_count += 1
    ...
    def em_step(self, corpus, parser_class, normalization_groups, bitext=False):
        ...
        for chart in parse_generator:
            i += 1
            if not chart:
                print("No parse for sentence %d." % i)
                continue
    ...
    def kbest(self, k):
        ...
        while True:
            ...
            if not next_derivation.frontier:  # This is the next best complete derivation
                ...
                kbest.append(
                    (next_derivation.weight, convert_derivation(next_derivation.derivation, "START")[1]["START"]))
                continue
